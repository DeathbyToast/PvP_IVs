...
function calculate(baseatk, basedef, basesta, floor, minLvl, maxLvl, invalid, league, mon) {
    ...
    for (var atk = floor / 1; atk <= 15; atk++) {
        for (var def = floor / 1; def <= 15; def++) {
            for (var sta = floor / 1; sta <= 15; sta++) {
                var solvedCPM = Math.sqrt((10 * league) / ((baseatk + atk) * Math.sqrt(basedef + def) * Math.sqrt(basesta + sta)));
                var testLvl = cpm.findIndex((element) => element > solvedCPM);
                if (testLvl > -1) {
                    testLvl = testLvl - 1;
                } else {
                    testLvl = 100; /* maxLvl case */
                }
                ...
                for (var level = maxLvl; level >= minLvl; level--) {
                    ...
                    if (atk === floor / 1 && def === floor / 1 && sta === floor / 1) { maxLvl = level; }

                    if (testLvl !== level) {
                        console.error("Mismatch! testLvl:" + testLvl + ", level:" + level + " for " + atk + "/" + def + "/" + sta + ", cp:" + cp);
                    }
    ...
    if (perfTiming) {
        T1 = performance.now();
        console.log(mon + " calculateFancy: " + (T1 - T0).toFixed(1) + "ms (Build " + (Tb - T0).toFixed(1) + "ms + Sort " + (Ts - Tb).toFixed(1) + "ms) for " + numRanks + " ranks.");
    }
    document.getElementById('timing_outputs').innerHTML += "calculateFancy: " + (T1 - T0).toFixed(1) + "ms (Build " + (Tb - T0).toFixed(1) + "ms + Sort " + (Ts - Tb).toFixed(1) + "ms) for " + numRanks + " ranks.<br>";
    return sorted;
}